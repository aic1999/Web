...
public class Youtu {
	...
	public final static String API_YOUTU_MY_END_POINT = "http://api.youtu.qq.com/";
	...
	throws IOException, JSONException, KeyManagementException, NoSuchAlgorithmException {
		...
		connection.setRequestProperty("Content-Length", "9999999999");
	 ...
	 * @Title: FaceFuse
	 * @Description: 人脸融合
	 * @date 2018年1月20日 下午4:33:34 
	 * @author niush
	 */
	public JSONObject FaceFuse(String image_url,String model) throws IOException, 
	JSONException, KeyManagementException, NoSuchAlgorithmException {
		JSONObject data = new JSONObject();
		
		//base64 编码后的输入图像
		StringBuffer image_data = new StringBuffer("");
		GetBase64FromFile(image_url, image_data);
		data.put("rsp_img_type", "url");
        data.put("img_data", image_data);
        
		//cmd 固定为"doFaceMerge"， model_id 通用模板id
		String opdata = "{\"cmd\": \"doFaceMerge\",\"params\": {\"model_id\": \""+model+"\"}}";
		//将opdata放入JSONObject中，过滤掉 "\"
		JSONObject myObject = new JSONObject(opdata);
		List myList = new ArrayList();
		myList.add(myObject);
		// 注意opdata是一个数组
		data.put("opdata", myList);
		
		System.out.println("1.请求开始，将参数封入json========");
		JSONObject response = m_not_use_https ? SendHttpRequest(data,  "cgi-bin/pitu_open_access_for_youtu.fcg") : SendHttpsRequest(data, "cgi-bin/pitu_open_access_for_youtu.fcg");
		return response;
	}
}
